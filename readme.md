...
Un mensaje de commit debe explicar **qué hace** el cambio, no _cómo_ lo hace. Tiene que ser legible para un compañero que ve el historial sin ver el código.
...
> R:R: Porque git revert no reescribe el historial compartido y es seguro después de hacer push, mientras que git reset --hard puede romper el historial de otros.
=======
**P1** — ¿Por qué `git revert` es preferible a `git reset --hard` cuando ya hiciste push de los cambios?
...
> R:Una usa módulo y la otra operaciones de bits. Ambas detectan si un número es par, pero la versión con & es más eficiente a bajo nivel.
